...
let code = fs.readFileSync('src/utils/unitConverter.ts', 'utf-8');

const replacement = `export const unitsData: Record<Category, Unit[]> = {
  Length: [
...
code = code.replace(/export const unitsData: Record\<Category, Unit\[\]\> = \{[\s\S]*?\n\};\n/, replacement + '\n');
fs.writeFileSync('src/utils/unitConverter.ts', code);
