...
const AnimatedStat = ({ stat, inView }: { stat: any, inView: boolean }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} transition={{ duration: 0.5, delay: stat.id * 0.1 }} className="flex flex-col items-center justify-center p-6 md:p-8 bg-white dark:bg-[#252525]/70 backdrop-blur-xl rounded-[32px] shadow-[0_8px_32px_rgba(0,0,0,0.04)] border border-slate-100 dark:border-[#333] text-center hover:-translate-y-1 transition-all">
      <div className="w-12 h-12 bg-[#0072de]/10 dark:bg-[#0072de]/20 rounded-[32px] flex items-center justify-center text-[#0072de] dark:text-[#3399ff] mb-4 border border-indigo-100/50">
        <stat.icon className="w-6 h-6" strokeWidth={2.5} />
      </div>
      <motion.div 
        animate={isFinished ? { scale: [1, 1.1, 1] } : { scale: 1 }}
        transition={{ duration: 0.4, ease: "easeInOut" }}
        className="text-4xl md:text-5xl lg:text-[3.5rem] font-black tracking-tighter text-slate-900 dark:text-white flex items-baseline font-heading">
...
export default function SocialProofSection() {
  ...
  return (
    <div className="w-full py-16 md:py-24 mb-16 bg-slate-50 dark:bg-[#121212] rounded-[3rem] border border-slate-200 dark:border-[#333] overflow-hidden relative">
      <div className="max-w-7rxl mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#0072de]/10 dark:bg-[#0072de]/20 text-[#0072de] dark:text-[#3399ff] rounded-full text-[10px] font-bold tracking-[0.2em] uppercase mb-6">Wall of Love</div>
          <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-slate-900 dark:text-white mb-6 font-heading">Trusted by the Top 1% of Engineers</h2>
          <p className="text-slate-600 dark:text-slate-400 font-medium text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">Join 24,847+ professionals across 15 countries relying on Civil Estimation Pro for precision and speed.</p>
        </div>
        ...
        <div ref={ref} className="grid grid-cols-2 lg:grid-cols-4 gap-5 md:gap-8 mb-24">
          ...
          <div className="track gap-6">
            {[...TESTIMONIALS.slice(0, 5), ...TESTIMONIALS.slice(0, 5)].map((t, idx) => (
              <div key={`row1-${idx}`} className="w-[350px] md:w-[400px] shrink-0 bg-white dark:bg-[#252525]/80 backdrop-blur-xl rounded-[32px] p-8 md:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-slate-100 dark:border-[#333] hover:-translate-y-1.5 transition-all">
                ...
                <p className="text-slate-800 dark:text-slate-200 mb-8 leading-[1.65] font-medium text-[15px]">"{t.quote}"</p>
                <div className="flex items-center gap-4 pt-6 border-t border-slate-100 dark:border-[#333]">
                  <div className="relative shrink-0">
                    <img src={t.avatar} alt={t.name} loading="lazy" className="w-12 h-12 rounded-full object-cover" />
                    <div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-white dark:bg-[#252525] text-[10px] shadow-sm">{t.flag}</div>
                  </div>
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white text-base leading-tight">{t.name}</div>
                    <div className="text-xs font-medium text-slate-500 mt-1">{t.title}, <span className="text-[#0072de] dark:text-[#3399ff]">{t.company}</span></div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="track track-reverse gap-6">
            {[...TESTIMONIALS.slice(5), ...TESTIMONIALS.slice(5)].map((t, idx) => (
              <div key={`row2-${idx}`} className="w-[350px] md:w-[400px] shrink-0 bg-white dark:bg-[#252525]/80 backdrop-blur-xl rounded-[32px] p-8 md:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-slate-100 dark:border-[#333] hover:-translate-y-1.5 transition-all">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((s) => ( <Star key={s} className="w-4 h-4 fill-[#F59E0B] text-[#F59E0B]" /> ))}
                    <span className="text-amber-600 font-bold ml-1.5 text-sm">{t.rating.toFixed(1)}</span>
          ...
          <div className="absolute inset-y-0 left-0 w-8 md:w-24 bg-gradient-to-r from-[#f8fafc] dark:from-[#121212] to-transparent pointer-events-none" />
          <div className="absolute inset-y-0 right-0 w-8 md:w-24 bg-gradient-to-l from-[#f8fafc] dark:from-[#121212] to-transparent pointer-events-none" />
        </div>
      </div>
    </div>
  );
