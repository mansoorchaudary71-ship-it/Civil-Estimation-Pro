...
const AbstractShape = ({ type }: { type: 'concrete' | 'steel' | 'architectural' | 'soil' | 'default' }) => {
  return (
    <motion.div 
      className="absolute -bottom-6 -right-6 md:-bottom-12 md:-right-12 w-48 h-48 md:w-64 md:h-64 opacity-20 pointer-events-none text-white blur-[1px]"
      whileHover={{ rotate: 15, scale: 1.1 }}
      transition={{ duration: 0.8, ease: "easeOut" }}
    >
      {type === 'concrete' && (
        <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M100 20L180 60V140L100 180L20 140V60L100 20Z" fill="url(#gradConcrete)" stroke="currentColor" strokeWidth="4"/>
          <path d="M100 20V100L180 60 M100 100V180 M20 60L100 100" stroke="currentColor" strokeWidth="4"/>
          <defs>
      ...
      {type === 'steel' && (
        <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M60 20H140V60H110V140H140V180H60V140H90V60H60V20Z" fill="url(#gradSteel)" stroke="currentColor" strokeWidth="4"/>
          <defs>
      ...
      {type === 'architectural' && (
        <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="100" cy="100" r="80" fill="url(#gradArch)" stroke="currentColor" strokeWidth="4"/>
          <path d="M100 20L180 180H20L100 20Z" stroke="currentColor" strokeWidth="4" fill="none"/>
          <defs>
      ...
      {(type === 'soil' || type === 'default') && (
        <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect x="40" y="40" width="120" height="120" rx="20" fill="url(#gradDef)" stroke="currentColor" strokeWidth="4" transform="rotate(45 100 100)"/>
          <defs>
      ...
      >
        ...
        >
          ...
          <div className="absolute top-4 right-4 flex gap-2 z-20">
            {mod.premium && (
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-black shadow-sm border border-white/20">
            ...
            {mod.isNew && (
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-[rgba(255,255,255,0.9)] text-black shadow-sm border border-white/20">
          ...
          <div className="flex flex-col z-10 h-full relative">
            <div className={`flex items-center justify-center ${size === 'large' ? 'w-16 h-16 rounded-2xl mb-6' : 'w-12 h-12 rounded-xl mb-5'} bg-[rgba(255,255,255,0.1)] border border-[rgba(255,255,255,0.2)] text-white shadow-inner group-hover:bg-white group-hover:text-black transition-colors duration-500`}>
              {React.createElement(IconComponent as React.ElementType, { className: size === 'large' ? 'w-8 h-8' : 'w-6 h-6' })}
            </div>
            
            <div className="mt-auto">
              <h4 className={`${size === 'large' ? 'text-2xl lg:text-3xl pr-12' : 'text-lg pr-4'} font-bold text-white mb-3 transition-colors leading-tight drop-shadow-md`}>
                {mod.title}
              </h4>
              <p className={`${size === 'large' ? 'text-sm lg:text-base max-w-[80%]' : 'text-xs max-w-[90%]'} text-[rgba(255,255,255,0.7)] font-medium leading-relaxed`}>
                {mod.description}
              </p>
            </div>
          </div>
          ...
          <div className="absolute right-0 bottom-0 w-32 h-32 bg-white mix-blend-screen opacity-0 group-hover:opacity-[0.05] blur-[40px] rounded-full transition-opacity duration-700 pointer-events-none"></div>
