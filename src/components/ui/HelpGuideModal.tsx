...
export function HelpGuideModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[12px] w-full max-w-lg shadow-2xl overflow-hidden flex flex-col relative max-h-[90vh]">
        
        <button onClick={onClose} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 bg-slate-100 dark:bg-[#6B46C1] rounded-full transition-colors">
          <X className="w-5 h-5" />
        </button>

        <div className="p-6 md:p-8 border-b border-[var(--border-color)]">
          <h2 className="text-[18px] font-bold flex items-center gap-3 text-[var(--text-primary)]">
            ...
            <div className="space-y-3">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#4B5563]">Find them on the home dashboard categorised by trade.</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#4B5563]">Click the sparkle icon on the search bar to ask AI for guidance.</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#4B5563]">Every tool allows exporting material breakdowns to PDF or Excel.</p>
                </div>
              </div>
            </div>
          </div>
          
          <div className="bg-slate-50 dark:bg-[#6B46C1]/50 p-4 rounded-[12px] border border-[var(--border-color)]">
            <h3 className="font-semibold text-sm text-[var(--text-primary)] mb-1">Your Progress</h3>
            <p className="text-xs text-[#4B5563] mb-2">You've explored {settings.usedTools?.length || 0} out of {ALL_TOOLS.length} tools.</p>
            <div className="w-full bg-slate-200 dark:bg-[#6B46C1] rounded-full h-2">
