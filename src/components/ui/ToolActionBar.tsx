...
export function ToolActionBar({ onHome, onHistory, onSave, onShare }: ToolActionBarProps) {
  return (
    <div className="mt-8 flex justify-center w-full pb-4 px-2">
      <div className="flex items-center gap-2 p-1.5 bg-bg-card border border-border-color rounded-full shadow-sm max-w-full overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        <button 
          onClick={onHome}
