        ...
        return (
          <button
            ...
            className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 min-w-[110px] ${
              isActive
                ? 'bg-white dark:bg-slate-900 text-[#1A1A1A] dark:text-indigo-400 shadow-sm ring-1 ring-slate-200/80 dark:ring-slate-700 scale-[1.02] z-10'
          ...
          >
            {option.icon && (
              <span className={`${isActive ? 'text-[#1A1A1A] dark:text-indigo-400' : 'text-slate-700 dark:text-slate-700'}`}>
                {option.icon}
              </span>
            )}
