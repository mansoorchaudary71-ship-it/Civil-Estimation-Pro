...
function parseAndFormat(strValue: string | number, title: string = '', unitNode: React.ReactNode = '') {
  ...
  const unitLower = (typeof unitNode === 'string' ? unitNode : suffix).toLowerCase();
  const origDecimalsMatch = numStr.match(/\.(\d+)/);
  let decimals = origDecimalsMatch ? origDecimalsMatch[1].length : 2;

  if (unitLower.includes('bag') || unitLower.includes('nos') || titleLower.includes('count') || titleLower.includes('bricks') || titleLower.includes('panels') || typeof unitNode === 'string' && unitNode.toLowerCase().includes('nos')) {
    decimals = 0;
  }
  else if (unitLower.includes('m³') || unitLower.includes('cu.m') || unitLower.includes('cft') || unitLower.includes('sq.m') || unitLower.includes('sft') || unitLower.includes('ton')) {
    decimals = 2;
  }
  else if (unitLower.includes('rs') || unitLower.includes('pkr') || titleLower.includes('cost') || titleLower.includes('price') || titleLower.includes('rate') || prefix.includes('$') || prefix.includes('Rs') || suffix.includes('Rs')) {
    if (titleLower.includes('pkr') || prefix.toLowerCase().includes('rs') || suffix.toLowerCase().includes('rs') || unitLower.includes('rs')) {
      decimals = 0; // Spec: "All currency in PKR must show: Rs 1,115,500" - 0 decimals
      if (!prefix.toLowerCase().includes('rs') && !suffix.toLowerCase().includes('rs') && !unitLower.includes('rs')) {
...
export function ResultCard({
  ...
  const isCurrency = parsed.isNumeric && (Object.keys(parsed).length > 0) && (title.toLowerCase().includes('cost') || title.toLowerCase().includes('price') || title.toLowerCase().includes('budget') || title.toLowerCase().includes('savings') || parsed.prefix.includes('Rs') || parsed.prefix.includes('$') || parsed.suffix.includes('Rs'));
  if (isCurrency && !unit?.toString().toLowerCase().includes('bag')) {
     const cleanSuffix = parsed.suffix.replace(/rs\.?/i, '').replace(/\$/i, '').trim();
    ...
    >
      ...
      <div className="flex flex-col w-full relative z-10 mt-1 overflow-hidden">
        <div className="flex flex-wrap items-baseline gap-1.5 sm:gap-2">
          ...
          {unit && (
            <span className="text-[13px] sm:text-sm font-semibold ml-1 shrink-0 text-slate-500 dark:text-slate-400">
              {unit}
            </span>
          )}
        </div>
