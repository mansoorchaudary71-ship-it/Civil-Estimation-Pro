...
export default function EarthworksEstimator() {
  ...
  const [calcMethod, setCalcMethod] = useState<"prismoidal" | "averageEnd">(
    "prismoidal",
  );
  ...
  const am =
    calcMethod === "prismoidal" && autoCalcAm
      ? Math.pow((Math.sqrt(a1) + Math.sqrt(a2)) / 2, 2)
      : inputAm;
  const solidVolume =
    calcMethod === "averageEnd"
      ? l * ((a1 + a2) / 2)
      : (l / 6) * (a1 + 4 * am + a2);
  ...
  const totalCostRaw =
    solidVolume * excRateParsed +
    compactedVolume * compRateParsed +
