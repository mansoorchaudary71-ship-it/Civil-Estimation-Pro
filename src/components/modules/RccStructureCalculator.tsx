...
export default function RccStructureCalculator({ isEmbedded = false }: { isEmbedded?: boolean }) {
  ...
  const [colWidth, setColWidth] = useState<string>("300"); // mm
  const [colDepth, setColDepth] = useState<string>("300"); // mm
  const [colDia, setColDia] = useState<string>("300"); // mm
  const [colMainBarDia, setColMainBarDia] = useState<string>("16"); // mm
  const [colTieDia, setColTieDia] = useState<string>("8"); // mm
  const [colTieSpacing, setColTieSpacing] = useState<string>("150"); // mm
  const [colCover, setColCover] = useState<string>("40"); // mm
  const [colRoundBarsCount, setColRoundBarsCount] = useState<string>("6");

  const structureTypes: { id: StructureType; label: string; icon: any; color: string }[] = [
    { id: "Simple Slab", label: "Simple Slab", icon: Layers, color: "text-blue-500 bg-blue-100 dark:bg-blue-500/20" },
    { id: "One Way Slab", label: "One Way Slab", icon: AlignVerticalSpaceAround, color: "text-indigo-500 bg-indigo-100 dark:bg-indigo-500/20" },
    { id: "Two Way Slab", label: "Two Way Slab", icon: Box, color: "text-cyan-500 bg-cyan-100 dark:bg-cyan-500/20" },
    { id: "4 Bar Column", label: "4 Bar Column", icon: Columns, color: "text-emerald-500 bg-emerald-100 dark:bg-emerald-500/20" },
    { id: "6 Bar Column", label: "6 Bar Column", icon: Columns, color: "text-teal-500 bg-teal-100 dark:bg-teal-500/20" },
    { id: "8 Bar Column", label: "8 Bar Column", icon: Columns, color: "text-amber-500 bg-amber-100 dark:bg-amber-500/20" },
    { id: "Round Column", label: "Round Column", icon: Circle, color: "text-orange-500 bg-orange-100 dark:bg-orange-500/20" },
  ];

  const calculate = () => {
    let concreteVol = 0; // m³
    let totalSteelKg = 0; // kg
    ...
    if (activeType.includes("Slab")) {
      ...
      let extraWt = 0;
      if (isTwoWay) {
        // Approximate extra weight for two-way bending / crank bars
        extraWt = mainWt * 0.15;
      }
      ...
      // Columns
      const H = parse(colHeight);
      const W = parse(colWidth) / 1000; // m
      const D = parse(colDepth) / 1000; // m
      const diaM = parse(colDia) / 1000; // m
      const c = parse(colCover) / 1000; // m
      const mainDia = parse(colMainBarDia);
      ...
      else if (activeType === "Round Column") noOfBars = parse(colRoundBarsCount);
      ...
      if (activeType === "Round Column") {
        ...
        const tiesCount = Math.ceil(H / (tieSpc || 1)) + 1;
        const tieLen = 2 * ((W - 2 * c) + (D - 2 * c)) + 0.1; // 100mm for hook
        const tieWt = tiesCount * tieLen * (tieDia * tieDia / 162.28);

        let extraTiesWt = 0;
        if (noOfBars > 4) {
             // For 6 or 8 bars, more ties are often required (cross ties or diamond)
      ...
      <div className={isEmbedded ? "w-full" : "max-w-6xl mx-auto"}>
        {!isEmbedded && (
          <>
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium">Estimate concrete volume and steel reinforcement for standard structural elements.</p>
          </>
        )}
          ...
          {structureTypes.map((t) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center gap-2 px-2 py-4 rounded-[16px] border transition-all overflow-hidden group ${isActive ? 'bg-indigo-600 border-indigo-500 text-white shadow-xl -translate-y-1' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:-translate-y-1 hover:shadow-lg'}`}
              >
                {isActive && <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent opacity-50" />}
                <div className={`p-2.5 rounded-2xl ${isActive ? 'bg-white/20 text-white shadow-inner' : t.color} transition-all duration-300 group-hover:scale-110`}>
                  <Icon className="w-5 h-5" strokeWidth={isActive ? 2.5 : 2} />
                </div>
                <span className="text-[10px] sm:text-[11px] font-extrabold text-center leading-tight tracking-wide z-10">{t.label}</span>
              </button>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="font-bold text-xl mb-6">{activeType} Parameters</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              {!activeType.includes("Slab") && (
                <>
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Column Height (m)</label>
                    <input type="number" value={colHeight} onChange={e => setColHeight(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Clear Cover (mm)</label>
                    <input type="number" value={colCover} onChange={e => setColCover(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  
                  {activeType === "Round Column" ? (
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Diameter (mm)</label>
                      <input type="number" value={colDia} onChange={e => setColDia(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                    </div>
                  ) : (
                    <>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">Width (mm)</label>
                        <input type="number" value={colWidth} onChange={e => setColWidth(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-500 uppercase">Depth (mm)</label>
                        <input type="number" value={colDepth} onChange={e => setColDepth(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                      </div>
                    </>
                  )}
                  ...
                  {activeType === "Round Column" && (
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">No. of Main Bars</label>
                      <input type="number" value={colRoundBarsCount} onChange={e => setColRoundBarsCount(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                    </div>
                  )}

                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Main Bar Dia (mm)</label>
                    <input type="number" value={colMainBarDia} onChange={e => setColMainBarDia(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Stirrup / Tie Dia (mm)</label>
                    <input type="number" value={colTieDia} onChange={e => setColTieDia(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Tie Spacing (mm)</label>
                    <input type="number" value={colTieSpacing} onChange={e => setColTieSpacing(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border-none p-4 rounded-xl mt-1.5 font-bold focus:ring-2 focus:ring-indigo-500" />
                  </div>
                </>
              )}
            </div>
          ...
          <div className="lg:col-span-5 bg-indigo-950 border border-indigo-900 rounded-[2rem] p-6 lg:p-10 text-white shadow-2xl flex flex-col justify-between">
            <div>
               <h3 className="font-bold text-indigo-300 text-sm uppercase tracking-widest mb-8">Calculated Results</h3>
               
               <div className="space-y-6">
                 <div className="bg-indigo-900/60 p-6 rounded-2xl border border-indigo-800">
                    <span className="block text-indigo-300 text-xs font-bold uppercase mb-2">Concrete Volume</span>
                    <span className="text-4xl font-black text-white">
                      {concreteVol.toFixed(3)}
                      <span className="text-xl text-indigo-400 ml-2">m³</span>
                    </span>
                 </div>
                 
                 <div className="bg-indigo-900/60 p-6 rounded-2xl border border-indigo-800">
                    <span className="block text-indigo-300 text-xs font-bold uppercase mb-2">Total Steel Weight</span>
                    <span className="text-5xl font-black text-emerald-400">
                      {totalSteelKg.toFixed(2)}
                      <span className="text-2xl text-indigo-400 ml-3">kg</span>
                    </span>
