...
export default function SewerageEstimator() {
  ...
  const [trenchProfile, setTrenchProfile] = useState<'vertical' | 'sloped'>('vertical');
  const [trenchSlopeRatio, setTrenchSlopeRatio] = useState<string>('0.5'); // Horizontal : 1 Vertical
  ...
  const [flowDia, setFlowDia] = useState<string>('0.3'); // m
  const [flowGradient, setFlowGradient] = useState<string>('200'); // 1 in X
  const [flowMaterial, setFlowMaterial] = useState<'pvc' | 'concrete' | 'cast_iron' | 'clay'>('concrete');

  // Pipe Bedding State
  const [beddingCalcLength, setBeddingCalcLength] = useState<string>('100');
  ...
  const [beddingType, setBeddingType] = useState<'classA' | 'classB' | 'classC'>('classB');
  ...
  // Septic System State
  const [septicUsers, setSepticUsers] = useState<string>('5');
  ...
  const pipeVol = Math.PI * Math.pow(pipeOD / 2, 2) * tL;

  // Bedding Volume
  let topBeddingWidth = tW;
  if (trenchProfile === 'sloped') {
  ...
  const fD = parseFloat(flowDia) || 0;
  const fG = parseFloat(flowGradient) || 1;
  let n = 0.013; // default concrete
  if (flowMaterial === 'pvc') n = 0.009;
  ...
  const flowVelocity = (n > 0 && fD > 0 && fG > 0) ? (1 / n) * Math.pow(hydraulicRadius, 2/3) * Math.pow(slope, 1/2) : 0;
  const flowArea = Math.PI * Math.pow(fD / 2, 2);
  const dischargeCapacityM3 = flowArea * flowVelocity; // m3/s
  const dischargeCapacityL = dischargeCapacityM3 * 1000; // L/s

  // Structural Pipe Bedding Calculations
  const calcBedLength = parseFloat(beddingCalcLength) || 0;
  ...
  if (calcBedPipeOD > 0) {
    ...
    const h = Math.min(Math.max(calcBedHaunch, 0), calcBedPipeOD); // clamp haunch height
  ...
  // Cross-sectional area = Trench Width * Total Depth - Pipe DisplacedArea
  const totalBedDepth = calcBedUnder + Math.min(calcBedPipeOD, Math.max(0, calcBedHaunch));
  const bedCrossSection = Math.max(0, calcBedWidth * totalBedDepth - bedPipeDisplaced);
  const beddingCalculatedVol = bedCrossSection * calcBedLength;
  const beddingWeightTons = (beddingType === 'classB' || beddingType === 'classC') ? beddingCalculatedVol * 1.6 : 0; // 1600 kg/m3 = 1.6 t/m3

  // Septic System Calculations
  const sUsers = parseFloat(septicUsers) || 0;
  const sDemand = parseFloat(septicDemand) || 0;
  const sPerc = parseFloat(septicPercolation) || 1; // L/m2/day
  
  const dailyFlowLiters = sUsers * sDemand;
  const dailyFlowM3 = dailyFlowLiters / 1000;
  
  // Volume: 24h retention + sludge (assume ~40L per user per year for 2 years = 80L/user)
  const sludgeVolLiters = sUsers * 80;
  const septicTotalVolM3 = (dailyFlowLiters + sludgeVolLiters) / 1000;
  
  // Dimensions (L = 2W) => 2W^2 * D = V -> W = sqrt(V / 2D)
  const septicDepth = Math.max(1, Math.min(2.5, Math.pow(septicTotalVolM3, 1/3))); // dynamic depth between 1m and 2.5m
  const septicWidth = septicTotalVolM3 > 0 ? Math.sqrt(septicTotalVolM3 / (2 * septicDepth)) : 0;
  const septicLength = septicWidth * 2;
  
  const soakageAreaRequired = dailyFlowLiters / (sPerc > 0 ? sPerc : 1);
  const soakageDepth = Math.max(1.5, septicDepth + 0.5); // soakage pit slightly deeper than tank
  const soakageDia = soakageAreaRequired > 0 ? soakageAreaRequired / (Math.PI * soakageDepth) : 0;
  ...
  const handleAddPipesToBOQ = () => {
    ...
    if (existing) {
        ...
        // Also clean up any other length so they don't stack up
        const otherExisting = boqItems.find(item => item.desc.startsWith("Sewerage RCC Pipe"));
        if (otherExisting) {
            updateBoqItem(otherExisting.id, { qtyOverride: pipeCount, desc: descStr });
        } else {
            addBoqItem({ desc: descStr, unit: "nos", rate: 2500, qtyOverride: pipeCount });
        }
    }
  };
              ...
              <div className="px-6 pb-6 border-t border-gray-50 pt-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="grid grid-cols-2 gap-4 h-fit">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Bottom Width (m)</label>
                      ...
                      <div className="col-span-2 mt-2 border-t border-gray-100 pt-4">
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Trench Profile</label>
                      ...
                      {trenchProfile === 'sloped' && (
                        <div className="col-span-2 -mt-2">
                           <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Side Slope Ratio (Horizontal:Vertical)</label>
                           <input type="number" step="0.1" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50" placeholder="e.g. 0.5 for 1H:2V" value={trenchSlopeRatio} onChange={e => setTrenchSlopeRatio(e.target.value)} />
                        </div>
                      )}
                      
                      <div className="col-span-2 mt-2 border-t border-gray-100 pt-4">
                        <h3 className="text-sm font-bold text-gray-800 mb-3">Backfill Parameters</h3>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Pipe Outer Dia (m)</label>
                            <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50" value={pipeOuterDiameter} onChange={e => setPipeOuterDiameter(e.target.value)} />
                          </div>
                          <div>
                            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Bedding Depth (m)</label>
                            <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50" value={beddingDepth} onChange={e => setBeddingDepth(e.target.value)} />
                          </div>
                        </div>
                      </div>
                   </div>
                   <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 flex flex-col justify-center space-y-4">
                      <h3 className="text-amber-800 font-bold border-b border-amber-200 pb-2">Excavation & Backfill</h3>
                      
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-amber-700 font-medium">Total Excavation Vol</span>
                        <span className="text-amber-900 font-bold text-xl">{trenchVol.toFixed(2)} m³</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-amber-700 font-medium">Pipe Displacement Vol</span>
                        <span className="text-amber-900 font-bold">{pipeVol.toFixed(2)} m³</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-amber-700 font-medium">Bedding Volume</span>
                        <span className="text-amber-900 font-bold">{beddingVol.toFixed(2)} m³</span>
                      </div>
                      
                      <div className="flex justify-between items-center text-sm border-t border-amber-200 pt-3 mt-2">
                        <span className="text-amber-800 font-bold text-base">Net Backfill Volume</span>
                        <span className="text-amber-900 font-black text-2xl">{netBackfillVol.toFixed(2)} m³</span>
                      </div>
                   </div>
                 </div>
              </div>
          ...
          <div className="bg-white rounded-[1.5rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden mb-8">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-800">Pipe Bedding Calculator</h2>
              </div>
              <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${openSection === 'bedding' ? 'rotate-180' : ''}`} />
            </button>
            
            <div className={`transition-all duration-500 ease-in-out ${openSection === 'bedding' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
              <div className="px-6 pb-6 border-t border-gray-50 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-4 h-fit">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Trench Length (m)</label>
                          <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50" value={beddingCalcLength} onChange={e => setBeddingCalcLength(e.target.value)} />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Trench Width (m)</label>
                          <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50" value={beddingCalcWidth} onChange={e => setBeddingCalcWidth(e.target.value)} />
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Pipe Outer Dia (m)</label>
                        <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50" value={beddingCalcPipeOD} onChange={e => setBeddingCalcPipeOD(e.target.value)} />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Bedding Type / Class</label>
                        <select 
                        ...
                        >
                          <option value="classA">Class A (Concrete Cradle/Arch)</option>
                          <option value="classB">Class B (Granular Bedding)</option>
                          <option value="classC">Class C (Granular Shaped Bottom)</option>
                        </select>
                      </div>
                      <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                        <div>
                          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Depth Under Pipe (m)</label>
                          <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50" value={beddingUnderPipe} onChange={e => setBeddingUnderPipe(e.target.value)} />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Haunching HT (m)</label>
                          <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50" value={beddingHaunchHeight} onChange={e => setBeddingHaunchHeight(e.target.value)} />
                        </div>
                      </div>
                   </div>
                   <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100 flex flex-col justify-center space-y-4">
                      <h3 className="text-purple-800 font-bold border-b border-purple-200 pb-2">Material Requirements</h3>
                      
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-purple-700 font-medium">Cross-Sectional Area</span>
                        <div className="text-right">
                          <span className="text-purple-900 font-bold ">{bedCrossSection.toFixed(4)} </span>
                          <span className="text-purple-600 font-medium text-xs">m²</span>
                        </div>
                      </div>

                      <div className="flex justify-between items-center text-sm">
                        <span className="text-purple-700 font-medium">Total Volume</span>
                        <div className="text-right">
                          <span className="text-purple-900 font-bold text-xl">{beddingCalculatedVol.toFixed(2)} </span>
                          <span className="text-purple-600 font-medium text-sm">m³</span>
                        </div>
                      </div>

                      {(beddingType === 'classB' || beddingType === 'classC') && beddingWeightTons > 0 && (
                        <div className="flex justify-between items-center text-sm border-t border-purple-200 pt-3 mt-1">
                          <div className="flex flex-col">
                            <span className="text-purple-700 font-medium">Estimated Weight</span>
                            <span className="text-purple-500 text-[10px]">@ 1600 kg/m³ density</span>
                          </div>
                          <div className="text-right">
                            <span className="text-purple-900 font-black text-2xl">{beddingWeightTons.toFixed(2)} </span>
                            <span className="text-purple-600 font-bold">tons</span>
                          </div>
                        </div>
                      )}
                      ...
                      {beddingType === 'classA' && (
                        <div className="flex items-start gap-2 bg-indigo-50 border border-indigo-200 p-3 rounded-xl mt-1">
                           <p className="text-xs font-medium text-indigo-700">
                             For Class A (Concrete), you typically use low-strength concrete (e.g., M10 or M15).
                           </p>
                        </div>
                      )}
                   </div>
          ...
          <div className="bg-white rounded-[1.5rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-800">Hydraulic Flow Calculator</h2>
              </div>
              <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${openSection === 'flow' ? 'rotate-180' : ''}`} />
            </button>
            
            <div className={`transition-all duration-500 ease-in-out ${openSection === 'flow' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
              <div className="px-6 pb-6 border-t border-gray-50 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-4 h-fit">
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Pipe Inner Diameter (m)</label>
                        <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" value={flowDia} onChange={e => setFlowDia(e.target.value)} />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Gradient (1 in X)</label>
                        <input type="number" className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" value={flowGradient} onChange={e => setFlowGradient(e.target.value)} />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Pipe Material</label>
                        <select 
                        ...
                        >
                          <option value="pvc">PVC / Plastic (n=0.009)</option>
                          <option value="concrete">Concrete (n=0.013)</option>
                          <option value="cast_iron">Cast Iron (n=0.014)</option>
                          <option value="clay">Clay / Ceramic (n=0.015)</option>
                        </select>
                      </div>
                   </div>
                   <div className="bg-cyan-50 p-6 rounded-2xl border border-cyan-100 flex flex-col justify-center space-y-4">
                      <h3 className="text-cyan-800 font-bold border-b border-cyan-200 pb-2">Full-Bore Flow Characteristics</h3>
                      
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-cyan-700 font-medium">Flow Velocity</span>
                        <div className="text-right">
                          <span className="text-cyan-900 font-bold text-xl">{flowVelocity.toFixed(3)} </span>
                          <span className="text-cyan-600 font-medium text-sm">m/s</span>
                        </div>
                      </div>
                      ...
                      {flowVelocity > 0 && flowVelocity < 0.6 && (
                        <div className="flex items-start gap-2 bg-rose-50 border border-rose-200 text-rose-700 p-3 rounded-xl mt-1">
                           ...
                           <p className="text-xs font-medium">
                             Warning: Calculated velocity is below the typical self-cleansing velocity of 0.6 m/s. Siltation may occur.
                           </p>
                        </div>
                      )}

                      <div className="flex justify-between items-center text-sm">
                        <span className="text-cyan-700 font-medium">Discharge Capacity</span>
                        <div className="text-right">
                          <span className="text-cyan-900 font-bold text-lg">{dischargeCapacityM3.toFixed(4)} </span>
                          <span className="text-cyan-600 font-medium text-sm">m³/s</span>
                        </div>
                      </div>

                      <div className="flex justify-between items-center text-sm border-t border-cyan-200 pt-3 mt-1">
                        <span className="text-cyan-700 font-medium">Discharge (Liters/sec)</span>
                        <div className="text-right">
                          <span className="text-cyan-900 font-black text-2xl">{dischargeCapacityL.toFixed(2)} </span>
                          <span className="text-cyan-600 font-bold">L/s</span>
                        </div>
                      </div>
                   </div>
                </div>
          ...
          <div className="bg-white rounded-[1.5rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden mb-8">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-800">Septic System Sizing Calculator</h2>
              </div>
              <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${openSection === 'septic' ? 'rotate-180' : ''}`} />
            </button>
            
            <div className={`transition-all duration-500 ease-in-out ${openSection === 'septic' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
              <div className="px-6 pb-6 border-t border-gray-50 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-4 h-fit">
                      <div>
                   ...
                   <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100 flex flex-col justify-center space-y-4">
                      <h3 className="text-emerald-800 font-bold border-b border-emerald-200 pb-2">Septic Tank Dimensions</h3>
                      
                      <div className="grid grid-cols-3 gap-2">
                         <div className="bg-white/60 p-3 rounded-xl border border-emerald-100 text-center">
                            <span className="block text-emerald-600 text-xs font-semibold mb-1">Length</span>
                            <span className="block text-emerald-900 font-bold text-lg">{septicLength.toFixed(2)} m</span>
                         </div>
                         <div className="bg-white/60 p-3 rounded-xl border border-emerald-100 text-center">
                            <span className="block text-emerald-600 text-xs font-semibold mb-1">Width</span>
                            <span className="block text-emerald-900 font-bold text-lg">{septicWidth.toFixed(2)} m</span>
                         </div>
                         <div className="bg-white/60 p-3 rounded-xl border border-emerald-100 text-center">
                            <span className="block text-emerald-600 text-xs font-semibold mb-1">Liquid Depth</span>
                            <span className="block text-emerald-900 font-bold text-lg">{septicDepth.toFixed(2)} m</span>
                         </div>
                      </div>
                      <p className="text-[11px] text-emerald-600 font-medium text-center">Calculated Volume: {septicTotalVolM3.toFixed(2)} m³ (Includes 24hr retention + sludge)</p>

                      <h3 className="text-emerald-800 font-bold border-b border-emerald-200 pb-2 mt-2">Soakage Pit Dimensions</h3>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-emerald-700 font-medium">Req. Sidewall Area</span>
                        <span className="text-emerald-900 font-bold text-base">{soakageAreaRequired.toFixed(2)} m²</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 mt-1">
                         <div className="bg-white/60 p-3 rounded-xl border border-emerald-100 text-center">
                            <span className="block text-emerald-600 text-xs font-semibold mb-1">Diameter</span>
                            <span className="block text-emerald-900 font-bold text-xl">{soakageDia.toFixed(2)} m</span>
                         </div>
                         <div className="bg-white/60 p-3 rounded-xl border border-emerald-100 text-center">
                            <span className="block text-emerald-600 text-xs font-semibold mb-1">Depth</span>
                            <span className="block text-emerald-900 font-bold text-xl">{soakageDepth.toFixed(2)} m</span>
                         </div>
                      </div>
                   </div>
                </div>
