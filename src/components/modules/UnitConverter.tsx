          ...
          {categories.map((c) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center gap-3 p-4 rounded-[20px] border transition-all overflow-hidden group ${isActive ? 'bg-fuchsia-600 border-fuchsia-500 text-white shadow-xl shadow-fuchsia-600/30 -translate-y-1' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:-translate-y-1 hover:shadow-lg'}`}
              >
                {isActive && <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent opacity-50" />}
                <div className={`p-3 rounded-2xl ${isActive ? 'bg-white/20 text-white shadow-inner' : c.color} transition-all duration-300 group-hover:scale-110`}>
                  <Icon className="w-7 h-7" strokeWidth={isActive ? 2.5 : 2} />
                </div>
                <span className="text-[11px] sm:text-xs font-extrabold tracking-wide z-10 text-center leading-tight">{c.label}</span>
              </button>
            )
          })}
        </div>
