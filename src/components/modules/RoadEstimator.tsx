...
export default function RoadEstimator() {
  ...
  const [shoulderWidth, setShoulderWidth] = useState<string>('1.5'); // meters
  const [shoulderFall, setShoulderFall] = useState<string>('3.0'); // %
  const [sideSlope, setSideSlope] = useState<string>('1.5'); // H:1V
  ...
  const [primeRateInput, setPrimeRateInput] = useState<string>('1.0'); // Liters/m²
  const [tackRateInput, setTackRateInput] = useState<string>('0.25'); // Liters/m²

  const [rateSg, setRateSg] = useState<string>(''); // per m3
  const [rateSb, setRateSb] = useState<string>(''); // per m3
  const [rateWbm, setRateWbm] = useState<string>(''); // per m3
  const [rateAsp, setRateAsp] = useState<string>(''); // per ton
  const [ratePrime, setRatePrime] = useState<string>(''); // per ton
  const [rateTack, setRateTack] = useState<string>(''); // per ton

  const [asphaltDensity, setAsphaltDensity] = useState<string>('2.3'); // tons/m3
  const [bitumenSG, setBitumenSG] = useState<string>('1.01'); // specific gravity
  ...
  // Trapezoidal cross-section calculations considering side slope extending outwards
  const tA_m = aspT / 1000;
  const tW_m = wbmT / 1000;
  const tSB_m = sbT / 1000;
  const tSG_m = sgT / 1000;

  // Top/Bottom widths of each layer using side slope s = H/V
  // Note: V component is thickness so H = s * V
  const wAspTop = w + 2 * shW;
  ...
  // Cross-sectional Areas strictly following typical trapezoidal calculation: V = ((W_top + W_bot) / 2) * t * L
  const areaAsp = ((wAspTop + wAspBot) / 2) * tA_m;
  ...
  // Assumption: density in loose state -> Coarse(1.6), Screening(1.5), Binding(1.6), Sub-Base / GSB(1.6)
  const wbmCoarseTons = wbmCoarseVol * 1.6;
  ...
  const primeCoatRate = parseFloat(primeRateInput) || 0;
  ...
  // Convert Bitumen to Metric Tons (Liters to kg -> /1000)
  const primeCoatTons = primeCoatVolume * (parseFloat(bitumenSG) || 0) / 1000;
  const tackCoatTons = tackCoatVolume * (parseFloat(bitumenSG) || 0) / 1000;

  // Costs
  const costSg = volSG * (parseFloat(rateSg) || 0);
  ...
  // Visual dimensions
  const svgTotalHeight = 300;
  const fixedMinHeight = 25; // minimum visual height per layer to show text
  // Calculate relative heights
  const hSG = Math.max(fixedMinHeight, sgT * 0.2);
  ...
  // Normalize heights to fit nicely in 200px max total layer thickness space if they get too big
  const currentTotal = hSG + hSB + hWBM + hAsp;
  ...
  // Visual Camber formulation
  const visualCamberDrop = Math.min(30, Math.max(0, c * 5));
  ...
  // Visual Width Mapping
  const maxModelWidth = Math.max(wSgBot, 1);
  ...
  // Path formulations
  const pathAsp = `M ${500 - dxAspTop},${y0_S} L ${500 - dxCarriageway},${y0_Carriage} L 500,${y0_C} L ${500 + dxCarriageway},${y0_Carriage} L ${500 + dxAspTop},${y0_S} L ${500 + dxAspBot},${y1_S} L ${500 + dxCarriageway},${y1_Carriage} L 500,${y1_C} L ${500 - dxCarriageway},${y1_Carriage} L ${500 - dxAspBot},${y1_S} Z`;
    ...
    <div className="w-full h-full overflow-y-auto bg-gray-50 text-gray-900 font-sans p-6 md:p-8">
          ...
          <section className="lg:col-span-5 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="col-span-1">
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Carriage (m)</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-shadow"
                      value={width}
                  ...
                  <div className="col-span-1">
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1" title="Shoulder Width (m)">Shoulder (m)</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-shadow"
                      value={shoulderWidth}
                  ...
                  <div className="col-span-1">
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1" title="Shoulder Cross-fall (%)">Fall (%)</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-shadow"
                      value={shoulderFall}
                  ...
                  <div className="col-span-1">
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1" title="Side Slope (H:1V)">Slope (H:1V)</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-shadow"
                      value={sideSlope}
            ...
            <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
              <div className="flex items-center gap-3 mb-5 border-b border-gray-50 pb-4">
                ...
                <h2 className="text-xl font-bold tracking-tight text-gray-800">Application Rates (L/m²)</h2>
              </div>
            ...
            <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
              <div className="flex items-center gap-3 mb-5 border-b border-gray-50 pb-4">
                ...
                <h2 className="text-xl font-bold tracking-tight text-gray-800">Material Rates</h2>
              </div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-50">
                   <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Asphalt Density (t/m³)</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-600 text-sm rounded-xl px-3 py-2 focus:outline-none"
                      value={asphaltDensity} onChange={e => setAsphaltDensity(e.target.value)}
                    />
                   </div>
                   <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Bitumen Specific Gravity</label>
                    <input 
                      type="number" 
                      className="w-full bg-gray-50/50 border border-gray-200 text-gray-600 text-sm rounded-xl px-3 py-2 focus:outline-none"
                      value={bitumenSG} onChange={e => setBitumenSG(e.target.value)}
                    />
                   </div>
                </div>
              </div>
          ...
          <section className="lg:col-span-7 flex flex-col gap-6">
            ...
            <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] p-8 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col justify-between" style={{ minHeight: '360px' }}>
              ...
              <div className="relative z-10 flex-1 flex flex-col justify-center w-full mt-4">
                 <svg viewBox="0 0 1000 320" preserveAspectRatio="xMidYMid meet" className="w-full h-auto drop-shadow-2xl">
                    <defs>
                      ...
                      <marker id="arrow" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
                        <path d="M 0 2 L 10 5 L 0 8 z" fill="white" opacity="0.5" />
                      </marker>
                    </defs>

                    <g filter="url(#shadow)">
                      {/* Sub-grade */}
                      <path d={pathSg} fill="url(#sgGrad)" stroke="#5c4a3d" strokeWidth="2" />
                      
                      {/* Sub-base */}
                      <path d={pathSb} fill="url(#sbGrad)" stroke="#a1907d" strokeWidth="2" />

                      {/* WBM */}
                      <path d={pathWbm} fill="url(#wbmGrad)" stroke="#504337" strokeWidth="2" />

                      {/* Asphalt */}
                      <path d={pathAsp} fill="url(#aspGrad)" stroke="#111827" strokeWidth="2" />
                    </g>

                    {/* Center Reference Line */}
                    <line x1="500" y1="10" x2="500" y2={y4_C + 30} stroke="white" strokeWidth="2" strokeDasharray="8 8" opacity="0.4" />
                    
                    {/* Horizontal Reference Line for Camber */}
                    <line x1="0" y1={y0_C} x2="1000" y2={y0_C} stroke="#fbbf24" strokeWidth="2" strokeDasharray="10 10" opacity="0.6" />
                    
                    {/* Width Annotations */}
                    <path d={`M ${500 - dxAspTop},${y0_C - 15} L ${500 + dxAspTop},${y0_C - 15}`} stroke="white" strokeWidth="1.5" opacity="0.5" markerStart="url(#arrow)" markerEnd="url(#arrow)" />
                    <text x="500" y={y0_C - 22} fill="white" fontSize="14" textAnchor="middle" opacity="0.8">Top Width: {wAspTop.toFixed(2)}m</text>
                    
                    {/* Bottom Width Annotation */}
                    {s > 0 && (
                      <>
                        <path d={`M ${500 - dxSgBot},${y4_S + 25} L ${500 + dxSgBot},${y4_S + 25}`} stroke="#876f59" strokeWidth="1.5" opacity="0.5" markerStart="url(#arrow)" markerEnd="url(#arrow)" />
                        <text x="500" y={y4_S + 42} fill="#5c4a3d" fontSize="14" textAnchor="middle" opacity="0.8">Base Width: {wSgBot.toFixed(2)}m</text>
                      </>
                    )}

                    {/* Labels */}
                    <text x="500" y={y0_C + vAsp / 2 + 5} fill="white" fontSize="14" fontWeight="bold" textAnchor="middle" letterSpacing="1">ASPHALT ({aspT}mm)</text>
                    <text x="500" y={y1_C + vWBM / 2 + 5} fill="white" fontSize="14" fontWeight="bold" textAnchor="middle" letterSpacing="1" opacity="0.9">WBM ({wbmT}mm)</text>
                    <text x="500" y={y2_C + vSB / 2 + 5} fill="#3e342b" fontSize="14" fontWeight="bold" textAnchor="middle" letterSpacing="1">SUB-BASE ({sbT}mm)</text>
                    <text x="500" y={y3_C + vSG / 2 + 5} fill="#1a140f" fontSize="14" fontWeight="bold" textAnchor="middle" letterSpacing="1">SUB-GRADE ({sgT}mm)</text>
                 </svg>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-amber-200 transition-colors">
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <Calculator className="w-4 h-4 text-amber-500" /> Layer Quantities
                </h3>
                <div className="space-y-3 font-mono text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                       <span className="font-bold text-gray-900 block">{volAsphalt.toFixed(2)} m³</span>
                       <span className="text-xs text-gray-400 font-medium">{asphaltTons.toFixed(2)} Tons</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-orange-200 transition-colors">
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <Droplets className="w-4 h-4 text-orange-500" /> Bitumen Coats
                </h3>
                <div className="space-y-4">
                   <div className="bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                      <div className="flex justify-between items-start mb-1">
                        <div className="text-orange-800 text-xs font-semibold">Prime Coat (on WBM)</div>
                        <div className="text-orange-600/70 text-[10px] font-bold">@ {primeCoatRate} L/m²</div>
                      </div>
                      <div className="flex items-end justify-between">
                         <div className="flex items-end gap-2">
                           <span className="text-xl font-black text-orange-600 leading-none">{primeCoatVolume.toFixed(1)}</span>
                           <span className="text-xs font-medium text-orange-400 mb-0.5">L</span>
                         </div>
                         <div className="text-sm font-bold text-orange-800 bg-orange-100/50 px-2 rounded">{primeCoatTons.toFixed(2)} T</div>
                      </div>
                   </div>
                   <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                      <div className="flex justify-between items-start mb-1">
                        <div className="text-gray-600 text-xs font-semibold">Tack Coat</div>
                        <div className="text-gray-400 text-[10px] font-bold">@ {tackCoatRate} L/m²</div>
                      </div>
                      <div className="flex items-end justify-between">
                         <div className="flex items-end gap-2">
                           <span className="text-xl font-black text-gray-800 leading-none">{tackCoatVolume.toFixed(1)}</span>
                           <span className="text-xs font-medium text-gray-500 mb-0.5">L</span>
                         </div>
                         <div className="text-sm font-bold text-gray-800 bg-gray-200/50 px-2 rounded">{tackCoatTons.toFixed(2)} T</div>
                      </div>
                   </div>
                </div>
              </div>
            ...
            <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-indigo-200 transition-colors mb-6 mt-6">
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                <Layers className="w-4 h-4 text-indigo-500" /> Granular Material Breakdown (Loose)
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-mono text-sm">
                <div>
                  <h4 className="font-semibold text-gray-700 bg-gray-50 px-3 py-1.5 rounded-md inline-block mb-3">WBM Sub-components</h4>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                      <span className="text-gray-600">Coarse Aggregate (80%)</span>
                      <div className="text-right">
                        <span className="font-bold text-gray-900 block">{wbmCoarseVol.toFixed(2)} m³</span>
                        <span className="text-xs text-gray-400 font-medium">{wbmCoarseTons.toFixed(2)} T</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                      <span className="text-gray-600">Screenings (15%)</span>
                      <div className="text-right">
                        <span className="font-bold text-gray-900 block">{wbmScreeningVol.toFixed(2)} m³</span>
                        <span className="text-xs text-gray-400 font-medium">{wbmScreeningTons.toFixed(2)} T</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Binding Material (5%)</span>
                      <div className="text-right">
                        <span className="font-bold text-gray-900 block">{wbmBindingVol.toFixed(2)} m³</span>
                        <span className="text-xs text-gray-400 font-medium">{wbmBindingTons.toFixed(2)} T</span>
                      </div>
                    </div>
                  </div>
                </div>
                <div>
                  <h4 className="font-semibold text-gray-700 bg-gray-50 px-3 py-1.5 rounded-md inline-block mb-3">Sub-base Material</h4>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">GSB Aggregate (100%)</span>
                      <div className="text-right">
                        <span className="font-bold text-gray-900 block">{sbLooseVol.toFixed(2)} m³</span>
                        <span className="text-xs text-gray-400 font-medium">{sbMaterialTons.toFixed(2)} T</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-emerald-200 transition-colors">
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
              ...
              <div className="space-y-3 font-mono text-sm">
                <div className="grid grid-cols-2 gap-4 pb-2 border-b border-gray-50">
                   <div className="flex justify-between">
                     <span className="text-gray-600">Sub-Grade</span>
                     <span className="font-bold text-gray-900">{formatCurrency(costSg)}</span>
                   </div>
                   <div className="flex justify-between">
                     <span className="text-gray-600">Sub-Base</span>
                     <span className="font-bold text-gray-900">{formatCurrency(costSb)}</span>
                   </div>
                </div>
                <div className="grid grid-cols-2 gap-4 pb-2 border-b border-gray-50">
                   <div className="flex justify-between">
                     <span className="text-gray-600">WBM</span>
                     <span className="font-bold text-gray-900">{formatCurrency(costWbm)}</span>
                   </div>
                   <div className="flex justify-between">
                     <span className="text-gray-600">Asphalt</span>
                     <span className="font-bold text-emerald-600">{formatCurrency(costAsp)}</span>
                   </div>
                </div>
                <div className="grid grid-cols-2 gap-4 pb-4">
                   <div className="flex justify-between">
                     <span className="text-gray-600">Prime Coat</span>
                     <span className="font-bold text-gray-900">{formatCurrency(costPrime)}</span>
                   </div>
                   <div className="flex justify-between">
                     <span className="text-gray-600">Tack Coat</span>
                     <span className="font-bold text-gray-900">{formatCurrency(costTack)}</span>
                   </div>
                </div>
                <div className="pt-3 border-t-2 border-slate-800 flex justify-between items-center text-lg">
                  <span className="font-bold text-slate-800 tracking-tight uppercase text-xs">Total Estimate Project Cost</span>
                  <span className="font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg">{formatCurrency(totalCost)}</span>
                </div>
              </div>
            </div>
      ...
      <ShareButtonWithPopup 
        ...
        exportFormat={{
          ...
          customTableData: [
            { item: "Sub-Grade", quantityStr: volSG.toFixed(2), unitStr: "m³", rate: rateSg, cost: costSg, color: "#64748b" },
            { item: "Sub-Base", quantityStr: volSB.toFixed(2), unitStr: "m³", rate: rateSb, cost: costSb, color: "#94a3b8" },
            { item: "WBM", quantityStr: volWBM.toFixed(2), unitStr: "m³", rate: rateWbm, cost: costWbm, color: "#cbd5e1" },
            { item: "Asphalt", quantityStr: asphaltTons.toFixed(2), unitStr: "tons", rate: rateAsp, cost: costAsp, color: "#1e293b" },
            { item: "Prime Coat", quantityStr: primeCoatTons.toFixed(2), unitStr: "tons", rate: ratePrime, cost: costPrime, color: "#f97316" },
            { item: "Tack Coat", quantityStr: tackCoatTons.toFixed(2), unitStr: "tons", rate: rateTack, cost: costTack, color: "#9ca3af" }
          ]
        }}
