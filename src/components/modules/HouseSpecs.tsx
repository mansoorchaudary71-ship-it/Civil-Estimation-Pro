    ...
    <div className="w-full h-full overflow-y-auto bg-[#fafafa] text-gray-900 font-sans p-6 md:p-8">
      <div className="max-w-6xl mx-auto space-y-8 pb-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* Brickwork Card */}
          <GlassCard title="Brickwork" icon={<Box className="w-5 h-5 text-red-500" />} color="red">
            <div className="space-y-4">
              <SelectInput 
                label="Brick Class" 
                value={specs.brickwork.type} 
                onChange={(v: string) => updateSpecs('brickwork', 'type', v)}
                options={[{ label: 'A-Class (Premium)', value: 'aClass' }, { label: 'B-Class (Standard)', value: 'bClass' }]} 
              />
              <NumberInput 
                label="Wastage (%)" 
                value={specs.brickwork.wastagePct.toString()} 
                onChange={(v: string) => updateSpecs('brickwork', 'wastagePct', parseFloat(v) || 0)} 
              />
            </div>
          </GlassCard>

          {/* Concrete / Structure */}
          <GlassCard title="Concrete Mix" icon={<Layers className="w-5 h-5 text-gray-500" />} color="gray">
            <div className="space-y-4">
              <SelectInput 
              ...
              <SelectInput 
                ...
                onChange={(v: string) => updateSpecs('concrete', 'foundationMix', v)}
                options={[
          ...
          {/* Plastering */}
          <GlassCard title="Plastering" icon={<Droplet className="w-5 h-5 text-teal-500" />} color="teal">
            <div className="space-y-4">
               <SelectInput 
                label="Mortar Ratio" 
                value={specs.plastering.mortarRatio} 
                onChange={(v: string) => updateSpecs('plastering', 'mortarRatio', v)}
                options={[
              ...
              <div className="grid grid-cols-2 gap-3">
                <NumberInput 
                  ...
                  onChange={(v: string) => updateSpecs('plastering', 'innerThickness', v)} 
                />
                <NumberInput 
                  label="Outer (in)" 
                  value={specs.plastering.outerThickness} 
                  onChange={(v: string) => updateSpecs('plastering', 'outerThickness', v)} 
                />
              </div>
            </div>
          </GlassCard>
          ...
          {/* Flooring */}
          <GlassCard title="Flooring" icon={<LayoutGrid className="w-5 h-5 text-amber-500" />} color="amber">
            <div className="space-y-4">
               <SelectInput 
                label="Material Type" 
              ...
              <NumberInput 
                ...
                onChange={(v: string) => updateSpecs('flooring', 'pricePerSqft', v)} 
              />
            </div>
          </GlassCard>

          {/* Ceiling */}
          <GlassCard title="Ceiling Finish" icon={<ArrowUpRight className="w-5 h-5 text-indigo-500" />} color="indigo">
            <div className="space-y-4 pb-4">
               ...
               <p className="text-[11px] text-gray-500 mt-4 leading-relaxed font-medium">
                 {specs.ceiling.type === 'pop' && "Standard Plaster of Paris finishes offering smooth surfaces."}
                 {specs.ceiling.type === 'gypsum' && "False ceiling boards. Ideal for recessed / hidden lighting."}
                 {specs.ceiling.type === 'bare' && "Bare concrete finish (Industrial look), zero added cost."}
               </p>
            </div>
          </GlassCard>

          {/* Doors & Windows */}
           <GlassCard title="Doors & Windows" icon={<AppWindow className="w-5 h-5 text-sky-500" />} color="sky">
            <div className="space-y-4">
               <SelectInput 
                label="Material" 
              ...
              <NumberInput 
                ...
                onChange={(v: string) => updateSpecs('doorsWindows', 'openingsSqft', v)} 
          ...
          {/* Paint & Finishes */}
          <GlassCard title="Paint & Finishes" icon={<Paintbrush className="w-5 h-5 text-fuchsia-500" />} color="fuchsia">
            <div className="space-y-4">
               <SelectInput 
          ...
          {/* Electrical & MEP */}
          <GlassCard title="Electrical & MEP" icon={<Sliders className="w-5 h-5 text-cyan-500" />} color="sky">
            <div className="space-y-4">
               <SelectInput 
                ...
                options={[
                  { label: '3/0.29 & 7/0.29 Standard', value: '3/0.29 & 7/0.29 Standard' },
              ...
              />
               <SelectInput 
                ...
                onChange={(v: string) => updateSpecs('electrical', 'switches', v)}
                options={[
...
function GlassCard({ title, icon, children, color }: { title: string, icon: React.ReactNode, children: React.ReactNode, color: string }) {
  ...
  return (
    <div>
      ...
      <input 
        ...
        onChange={(e) => onChange(e.target.value)}
      />
      {desc && <p className="text-[10px] text-gray-400 mt-1.5 font-bold tracking-wide">{desc}</p>}
    </div>
  );
}
    ...
    <div className="flex bg-gray-100/80 backdrop-blur-sm p-1 rounded-2xl w-full border border-gray-200/50">
      {options.map((opt: any) => (
        <button
          ...
          className={`flex-1 py-2.5 px-3 text-[12px] font-bold rounded-xl transition-all duration-300 ${
            value === opt.value 
