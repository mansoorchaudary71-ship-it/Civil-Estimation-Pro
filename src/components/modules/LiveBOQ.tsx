...
const COMMON_UNITS = [
  'm³', 'm²', 'm', 'sq.ft', 'sq.yd', 'cu.ft', 'ft', 'in', 'cm', 'mm', 'MT', 'count'
];

function BOQRow({ row, measurements, getQty, updateBoqItem, scalePxPerUnit, unitName, formatCurrency, settings }: any) {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const qty = getQty(row);
  const amount = qty * row.rate;
  const isLinked = row.linkedMeasurementIds && row.linkedMeasurementIds.length > 0;
  const useManual = isLinked ? !!row.isManualOverride : true;
  
  return (
    <tr className="hover:bg-slate-50 transition-colors group">
      <td className="py-2 px-3 font-mono text-blue-400 text-[10px]">{row.id}</td>
      ...
      <td className="py-2 px-3 text-slate-500">
        ...
        >
          ...
          {!COMMON_UNITS.includes(row.unit) && <option value={row.unit} className="bg-white">{row.unit}</option>}
        </select>
      </td>
      <td className="py-2 px-3 text-right">
        <div className="flex items-center justify-end gap-2 text-xs">
          {isLinked && (
            <button 
              onClick={() => updateBoqItem(row.id, { isManualOverride: !row.isManualOverride })}
              className={`p-1 rounded ${row.isManualOverride ? 'text-gray-500 hover:text-slate-800' : 'text-blue-400 bg-blue-400/10'}`}
              title={row.isManualOverride ? "Using manual override. Click to use linked." : "Using linked data. Click to override manually."}
            >
              {row.isManualOverride ? <Unlink className="w-3 h-3" /> : <LinkIcon className="w-3 h-3" />}
            </button>
          )}
          <div className="relative">
            {useManual ? (
              <input 
                ...
                onChange={(e) => updateBoqItem(row.id, { qtyOverride: parseFloat(e.target.value) || 0 })}
              />
            ) : (
              <span className="font-mono text-blue-400 px-1.5 py-0.5 inline-block min-w-[4rem] text-right">{qty.toFixed(2)}</span>
            )}
          </div>
        </div>
      </td>
      <td className="py-2 px-3 text-right relative">
        ...
        >
          <span className="truncate flex-1 text-left w-20">
            {isLinked ? `${row.linkedMeasurementIds?.length} linked` : "-- No Link --"}
          </span>
          ...
          <>
            ...
            <div className="absolute right-3 top-8 w-44 bg-white border border-slate-200 rounded-lg shadow-xl z-20 py-2 flex flex-col gap-1 max-h-48 overflow-y-auto">
              <div className="px-2 pb-1 border-b border-slate-200 mb-1">
                <span className="text-[10px] text-slate-500 font-semibold uppercase">Measurements</span>
              </div>
              {measurements.map((m: any) => {
                ...
                return (
                  <label key={m.id} className="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-50 cursor-pointer text-xs text-slate-800">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => {
                        const newIds = e.target.checked 
                          ...
                          : (row.linkedMeasurementIds || []).filter((id: string) => id !== m.id);
              ...
              {measurements.length === 0 && (
                <div className="px-3 py-2 text-xs text-slate-500">No measurements</div>
              )}
            </div>
          </>
        )}
...
export default function LiveBOQ() {
  ...
  const totalCost = boqItems.reduce((acc, row) => acc + (getQty(row) * row.rate), 0);
  ...
  const handleExport = async (type: 'pdf' | 'excel') => {
    ...
    if (type === 'pdf') {
       ...
       try {
         generatePDFReport(boqItems, measurements, scalePxPerUnit, unitName, details, settings.currency);
       } catch (error) {
       ...
       try {
         await generateExcelReport(boqItems, measurements, scalePxPerUnit, unitName, details, settings.currency);
       } catch (error) {
          ...
          <table className="w-full text-left text-xs">
            <thead className="text-slate-500 border-b border-slate-200">
              <tr>
                ...
                <th className="py-2 px-3 font-normal text-right w-24">Rate ({settings.currency})</th>
                <th className="py-2 px-3 font-normal text-right w-32">Amount ({settings.currency})</th>
              </tr>
            </thead>
            ...
            <tfoot className="border-t border-slate-200">
              <tr>
                <th colSpan={6} className="py-3 px-3 text-right text-[10px] text-slate-500 uppercase font-semibold">Total Estimated Cost</th>
                <th className="py-3 px-3 font-mono text-right text-sm text-green-500 font-bold">{formatCurrency(totalCost)}</th>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white border border-slate-200 rounded-lg p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-sm font-semibold mb-4 text-slate-900 flex items-center gap-2">
              <Download className="w-[16px] h-[16px] text-blue-400" /> Generate Professional Report
            </h3>
            
            <div className="flex flex-col gap-4 mb-6">
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-1.5">Project Name</label>
                <input 
                  type="text" 
                  value={exportData.projectName}
                  onChange={(e) => setExportData(prev => ({ ...prev, projectName: e.target.value }))}
              ...
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-1.5">Client Name</label>
                <input 
                  type="text" 
                  value={exportData.clientName}
                  onChange={(e) => setExportData(prev => ({ ...prev, clientName: e.target.value }))}
                  className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-800 focus:outline-none focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-1.5">Site Location</label>
                <input 
                  type="text" 
                  value={exportData.siteLocation}
                  onChange={(e) => setExportData(prev => ({ ...prev, siteLocation: e.target.value }))}
                  className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-800 focus:outline-none focus:border-blue-500"
                />
              </div>
            </div>
