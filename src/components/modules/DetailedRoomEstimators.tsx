  ...
  return (
    <div className="w-full flex justify-center animate-in fade-in slide-in-from-top-4 duration-500 pb-16">
      <div className="w-full max-w-6xl">
        <div className="flex gap-2 p-1 bg-white/50 backdrop-blur-md rounded-[24px] w-fit mx-auto mb-8 shadow-sm border border-slate-200 rounded-[24px] border border-slate-200 shadow-sm text-gray-800 overflow-x-auto max-w-full">
          {tabs.map(tab => (
            <button
              key={tab}
...
function ResultTable({ title, icon: Icon, items, onRateChange }: { title: string, icon: any, items: any[], onRateChange: (name: string, rate: number) => void }) {
  ...
  return (
    <div className="mb-8 last:mb-0">
      <h4 className="uppercase st mb-4 flex items-center gap-2 text-lg font-medium text-gray-800">
      ...
      <div className="bg-white shadow-sm border border-slate-200 rounded-[24px] overflow-hidden">
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200 text-gray-800">
              <tr>
                <th className="py-3 px-4 font-semibold text-gray-600">Item</th>
                <th className="py-3 px-4 font-semibold text-gray-600">Unit</th>
                <th className="py-3 px-4 font-semibold text-right text-gray-600">Qty</th>
                <th className="py-3 px-4 font-semibold text-right text-gray-600 w-32">Rate</th>
                <th className="py-3 px-4 font-semibold text-right text-gray-600">Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {items.map((item, i) => (
                <tr key={i} className="hover:bg-slate-50/50 text-gray-800">
                  <td className="py-3 px-4 font-medium text-gray-700">{item.name}</td>
                  <td className="py-3 px-4 text-gray-500 text-xs font-semibold">{item.unit}</td>
                  ...
                  <td className="py-2 px-4 min-w-[100px]">
                    <input
                      ...
                      min="0"
                      placeholder="0.00"
                      className="w-full bg-slate-100 border-none rounded-[16px] px-2 py-1.5 text-right sm: focus:ring-2 focus:ring-purple-500 focus:outline-none text-gray-800 min-h-[44px] text-base font-normal"
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <div className="lg:col-span-4 space-y-6">
        <h3 className=" text-lg font-medium text-gray-800 mb-4">{title} Estimator</h3>
      ...
      <div className="lg:col-span-8 bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100">
        <div className="flex justify-between items-end mb-8">
          ...
          <div className="text-right">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest block">Grand Total</span>
...
function CommonNumInput({ label, val, setVal, unit }: { label: string, val: string, setVal: (v:string)=>void, unit?: string }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="uppercase tracking-widest text-sm font-medium text-gray-700 mb-1 block">{label}</label>
      <div className="relative">
        <input 
          type="number" inputMode="decimal" 
          value={val} onChange={(e)=>setVal(e.target.value)}
          className="w-full bg-slate-50 border border-slate-200 rounded-[24px] px-4 py-2.5 text-sm font-semibold focus:ring-2 focus:ring-purple-500 outline-none transition-all text-gray-800"
        />
        {unit && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-600">{unit}</span>}
      </div>
    </div>
  );
}
...
function KitchenEstimator() {
  ...
  return <BaseEstimatorWrapper title="Kitchen" inputs={null} calculate={cb} renderInputs={() => (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2 border-b border-slate-200 pb-2 mb-2"><h4 className=" text-lg font-medium text-gray-800 mb-4">Room Dimensions</h4></div>
      ...
      <div className="col-span-2 border-b border-slate-200 pb-2 mt-4 mb-2"><h4 className=" text-lg font-medium text-gray-800 mb-4">Kitchen Specifics</h4></div>
...
function BasementEstimator() {
  ...
  return <BaseEstimatorWrapper title="Basement" inputs={null} calculate={cb} renderInputs={() => (
    <div className="grid grid-cols-2 gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="uppercase tracking-widest text-sm font-medium text-gray-700 mb-1 block">Soil Bulking Factor</label>
        <select value={bulkingF} onChange={e=>setBulkingF(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-[24px] px-4 py-2.5 text-sm font-semibold focus:ring-2 focus:ring-purple-500 outline-none transition-all text-gray-800">
