...
export default function FinishingEstimator() {
  ...
  /* Plaster inputs */ const [plasterThickness, setPlasterThickness] =
    useState<string>(isSI ? "12" : "0.5");
  const [mortarRatio, setMortarRatio] = useState<string>("4");
  /* 1:4 Tile inputs */ const [tileWidth, setTileWidth] = useState<string>(
    isSI ? "600" : "24",
  );
  const [tileLength, setTileLength] = useState<string>(isSI ? "600" : "24");
  const [tilesPerBox, setTilesPerBox] = useState<string>("4");
  /* pcs Paint inputs */ const [paintCoverage, setPaintCoverage] =
    useState<string>(isSI ? "12" : "350");
  useEffect(() => {
  ...
  const results = useMemo(() => {
    ...
    /* Plaster Work */ const pThickness = parseFloat(plasterThickness) || 12;
    /* mm */ const mRatio = parseFloat(mortarRatio) || 4;
    /* 1:mRatio */ const wetVolumeOption = (netArea * pThickness) / 1000;
    /* m3 */ const dryVolume = wetVolumeOption * 1.33;
    /* 33% extra for mortar */ const cementVolume = dryVolume / (1 + mRatio);
    const cementBags = Math.ceil((cementVolume * 1440) / 50);
    /* 1440 kg/m3 density of cement, 50kg/bag */ const sandVolumeCft =
      cementVolume * mRatio * 35.3147;
    /* Tile Work */ const tW = parseFloat(tileWidth) || 600;
    const tL = parseFloat(tileLength) || 600;
    const tpb = parseFloat(tilesPerBox) || 4;
    const tileArea = (tW * tL) / 1000000;
    /* m2 */ const totalTileAreaReq = netArea * 1.05;
    /* 5% wastage */ const numTiles =
      tileArea > 0 ? totalTileAreaReq / tileArea : 0;
    const boxesReq = tpb > 0 ? Math.ceil(numTiles / tpb) : 0;
    /* Paint Work */ const pc = parseFloat(paintCoverage) || 12;
  ...
  const costData = useMemo(() => {
    ...
    return [
      {
        name: "Cement",
        qty: `${results.cementBags.toFixed(1)} Bags`,
        value: costCement,
        color: "#64748b",
      },
      {
        name: "Sand",
        qty: `${results.sandVolumeCft} cft`,
        value: costSand,
        color: "#f59e0b",
      },
      {
        name: "Tiles",
        qty: `${Math.ceil(results.boxesReq)} Boxes`,
        value: costTiles,
        color: "#0ea5e9",
      },
      {
        name: "Paint",
        qty: `${results.paintLiters} L`,
        value: costPaint,
        color: "#ec4899",
      },
    ];
