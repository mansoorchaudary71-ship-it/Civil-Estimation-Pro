...
export default function PakistanCostCalculatorPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1E] font-sans pb-12">
      <div className="max-w-7xl mx-auto px-4 pt-12 md:pt-20 pb-16">
        ...
        <div className="mb-12">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            ...
            <span className="px-3 py-1.5 md:py-1 bg-slate-800 border border-slate-700/60 rounded-full text-[10px] uppercase tracking-wider font-bold text-slate-300 shadow-sm flex items-center gap-1.5">
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white tracking-tight mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="bg-[#0D1525] border border-slate-800 rounded-2xl p-6 md:p-8 shadow-sm">
              <div className="flex items-center gap-3 mb-8">
                 ...
                 <h2 className="text-xl font-bold text-white">Project Dimensions & Location</h2>
              </div>

              {/* Area & Unit */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Plot Area</label>
                  <input 
                    type="number" 
                    value={area} 
                    onChange={(e) => setArea(parseFloat(e.target.value) || 0)}
                    className="w-full bg-[#0A0F1E] border border-slate-700/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all font-semibold"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Measurement Unit</label>
                  <div className="flex bg-[#0A0F1E] p-1 rounded-xl border border-slate-700/50">
                     {Object.keys(unitMultipliers).map((u) => (
                       <button
                         key={u}
                         onClick={() => setUnit(u as keyof typeof unitMultipliers)}
                         className={`flex-1 text-xs py-2.5 rounded-lg font-bold transition-colors ${unit === u ? 'bg-amber-500 text-[#0A0F1E]' : 'text-slate-400 hover:text-white'}`}
                       >
                         {u}
                       </button>
                     ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                 <div>
                    ...
                    <select 
                       ...
                       className="w-full bg-[#0A0F1E] border border-slate-700/50 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all appearance-none font-semibold cursor-pointer"
                    >
                      {Object.keys(cityRates).map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                 </div>
                 <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Floors</label>
                    <div className="flex gap-2">
                       {Object.keys(floorModifiers).map(f => (
                          <button
                            key={f}
                            onClick={() => setFloors(f as keyof typeof floorModifiers)}
                            className={`flex-1 text-sm py-2.5 rounded-xl border font-bold transition-all ${floors === f ? 'bg-[#0A0F1E] border-amber-500 text-amber-500' : 'bg-[#0A0F1E] border-slate-700/50 text-slate-400 hover:border-slate-500 hover:text-white'}`}
                          >
                            {f}
                          </button>
                       ))}
              ...
              <button 
                ...
                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${hasBasement ? 'bg-amber-500/5 border-amber-500/40 text-amber-500' : 'bg-[#0A0F1E] border-slate-700/50 text-slate-400 hover:border-slate-600 hover:text-white'}`}
              >
            ...
            <div className="bg-[#0D1525] border border-slate-800 rounded-2xl p-6 md:p-8 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                 ...
                 <h2 className="text-xl font-bold text-white">Construction Quality</h2>
              </div>
                ...
                {Object.keys(cityRates["Lahore"]).map(type => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-5 rounded-xl border transition-all ${constructionType === type ? 'bg-amber-500/5 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.05)]' : 'bg-[#0A0F1E] border-slate-700/50 hover:border-slate-500'}`}
                    >
                      ...
                      <span className="text-sm font-mono text-slate-400 block mt-1">PKR {r[0].toLocaleString()} - {r[1].toLocaleString()} <span className="text-xs text-slate-500">/sqft</span></span>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="lg:col-span-5 h-full">
            <div className="bg-gradient-to-b from-[#152136] to-[#0D1525] border border-amber-500/20 rounded-3xl p-8 sticky top-28 shadow-xl flex flex-col h-full">
              
              <div className="mb-8">
                <p className="text-amber-500 font-bold text-xs uppercase tracking-widest mb-3">Estimated Total Cost</p>
                <h3 className="text-4xl lg:text-5xl font-black text-white tracking-tighter mb-2">
                ...
                <p className="text-slate-400 mt-4 text-sm font-medium flex justify-between items-center bg-[#0A0F1E]/50 rounded-lg py-2 px-3">
                  <span>Covered Area:</span>
                  <span className="text-white font-bold">{formatPKR(totalCoveredArea)} Sq.Ft</span>
                </p>
                <p className="text-slate-400 mt-2 text-sm font-medium flex justify-between items-center bg-[#0A0F1E]/50 rounded-lg py-2 px-3">
                 ...
                 <div className="space-y-3">
                    {getBreakdown(constructionType).map((item, idx) => (
                      <div key={idx} className="flex flex-col gap-1.5">
                         ...
                         <div className="w-full bg-[#0A0F1E] h-1.5 rounded-full overflow-hidden flex">
              ...
              {/* Disclaimer */}
              <div className="bg-[#0A0F1E] border border-amber-900/30 rounded-xl p-4 flex gap-3 items-start shrink-0">
                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                <p className="text-[11px] leading-relaxed text-slate-400">
                  <strong className="text-slate-300">Disclaimer:</strong> These are 2025 standard market rates. Exact costs vary based on design complexity, material inflation, transportation, and plot conditions. It does not include boundary wall, architect fee, or external utility connections.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mb-20">
           <h2 className="text-2xl md:text-3xl font-bold text-white mb-8 border-b border-slate-800/60 pb-4">
             Construction Cost Breakdown Pakistan (2025 Rates)
           </h2>
           <p className="text-slate-400 mb-8 max-w-4xl leading-relaxed">
           ...
           <div className="overflow-x-auto rounded-xl border border-slate-800 bg-[#0A0F1E] shadow-xl">
              <table className="w-full text-left text-sm text-slate-300">
                <thead className="bg-[#0D1525] text-slate-400 uppercase font-bold text-xs tracking-wider border-b border-slate-800">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  <tr className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4 font-bold text-white">Lahore</td>
                  ...
                  <tr className="hover:bg-slate-800/30 transition-colors bg-[#0D1525]/30">
                    <td className="px-6 py-4 font-bold text-white">Islamabad</td>
                  ...
                  <tr className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4 font-bold text-white">Karachi</td>
                  ...
                  <tr className="hover:bg-slate-800/30 transition-colors bg-[#0D1525]/30">
                    <td className="px-6 py-4 font-bold text-white">Other Cities (Avg)</td>
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { title: "Steel Weight Estimator", desc: "Calculate precise rebar weights & lengths.", icon: Grid, target: "#" }
                 ...
                 <a key={idx} href={tool.target} className="bg-[#0D1525] border border-slate-800 hover:border-amber-500/40 rounded-2xl p-5 group transition-all">
                    <div className="bg-slate-900/50 w-10 h-10 rounded-lg flex items-center justify-center mb-4 group-hover:bg-amber-500/10 transition-colors">
                      <tool.icon className="w-5 h-5 text-slate-400 group-hover:text-amber-500 transition-colors" />
                    </div>
                    <h4 className="text-white font-bold mb-1 group-hover:text-amber-50 transition-colors">{tool.title}</h4>
