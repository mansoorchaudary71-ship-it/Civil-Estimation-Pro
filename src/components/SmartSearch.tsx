      ...
      >
        <div className="pl-6 md:pl-8 pr-4 flex items-center justify-center h-full">
          {isSearching ? (
            ...
            <Search
              className={`w-5 h-5 md:w-6 md:h-6 transition-colors duration-300 ${isFocused || query ? "text-indigo-500" : "text-gray-600 "}`}
            />
          )}
        </div>
        <input
          ...
          className="w-full h-full bg-transparent border-none outline-none text-base md:text-lg font-semibold text-gray-800  placeholder-slate-400 px-2 cursor-text"
        />

        {/* Keyboard Shortcut Hint */}
        {!query && !isFocused && (
          <div className="hidden md:flex items-center justify-center mr-4">
            <span className="flex items-center justify-center w-6 h-6 rounded bg-slate-100  border border-slate-200  text-gray-600  text-xs font-bold font-mono">
              /
            </span>
          </div>
        )}
        ...
        <div className="mr-3 pr-2 flex items-center gap-2">
          <AnimatePresence>
            {query && (
              <motion.button
                ...
                onClick={clearSearch}
                className="p-2 rounded-full hover:bg-slate-100   text-gray-600  hover:text-gray-600 transition-colors duration-200"
              >
                <X className="w-5 h-5" />
              </motion.button>
            )}
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all duration-300 ${isListening ? "bg-red-50 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.3)] animate-pulse" : "hover:bg-[#FFFFFF]/10  text-gray-600  hover:text-indigo-500"}`}
          >
      ...
      <AnimatePresence>
        {showDropdown && (
          ...
          >
              ...
              {!query ? (
                ...
                <div className="space-y-6">
                  {recentSearches.length > 0 && (
                    <div>
                      <div className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-600  flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-2 px-3 mt-1">
                        {recentSearches.map((rec, i) => (
                          <button
                            ...
                            className="bg-slate-50  hover:bg-slate-100   border border-slate-200  text-gray-600 text-sm font-medium px-4 py-2 rounded-full transition-colors flex items-center gap-2"
                          >
                    ...
                    <div className="space-y-1 mt-1">
                      {SUGGESTED_SEARCHES.map((sugg, i) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-3 rounded-[32px] hover:bg-[#FFFFFF]/10  text-gray-600 hover:text-gray-900  text-sm font-medium transition-colors flex items-center justify-between group"
                        >
                          <span>"{sugg}"</span>
                          <ArrowUpRight className="w-4 h-4 text-gray-700 group-hover:text-indigo-400 transition-colors" />
                        </button>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 px-1">
                      {popularTools.map((mod) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-10 h-10 shrink-0 rounded-[32px] flex items-center justify-center bg-indigo-50 text-indigo-600 group-hover:bg-[#FFFFFF] group-hover:text-gray-900 transition-colors duration-300`}
                            >
                              {mod.icon ? (
                            ...
                            <div className="flex-1 w-full text-left">
                              ...
                              <p className="line-clamp-1 text-base font-normal text-gray-600 leading-relaxed">
                                {mod.category}
                              </p>
                            </div>
                          </div>
                ...
                <div className="space-y-4">
                  {localResults.length > 0 && (
                    <div>
                      <div className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-600 ">
                        Calculators & Tools
                      </div>
                      <div className="space-y-1 mt-1">
                        {localResults.map((result, idx) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-12 h-12 shrink-0 rounded-[32px] flex items-center justify-center bg-indigo-50 text-indigo-600 group-hover:bg-[#FFFFFF] group-hover:text-gray-900 transition-transform duration-300 shadow-sm border border-indigo-100 group-hover:border-transparent`}
                              >
                                {fullModuleData?.icon ? (
                              ...
                              <div className="flex-1 w-full text-left">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                                  <h4 className="group-hover: transition-colors text-lg font-medium text-gray-800 mb-4">
                                    {result.toolName}
                                  </h4>
                                  <span className="hidden sm:inline bg-slate-100  text-gray-500   text-xs font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                                    {result.category}
                                  </span>
                                </div>
                              ...
                              <ArrowUpRight className="hidden sm:block w-5 h-5 text-gray-700 group-hover:text-indigo-500 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {isCompletelyEmpty && (
                    <div className="text-center py-12 px-4">
                      ...
                      <p className="max-w-sm mx-auto mb-6 text-base font-normal text-gray-600 leading-relaxed">
                        We don't have a dedicated template for "{query}". Try
                        asking the AI Assistant instead.
                      </p>
                      <button
                        onClick={() => handleSelectTool("ai")}
                        className="bg-white  border border-[#FFFFFF] text-gray-900  font-bold py-3 px-6 rounded-[32px] inline-flex items-center gap-2 shadow-[0_8px_16px_rgba(15,23,42,0.15)] transition-all hover:bg-[#FFFFFF] hover:-translate-y-0.5 w-full sm:w-auto justify-center"
                      >
                  ...
                  {!isCompletelyEmpty && (
                    <div className="pt-2 border-t border-slate-100 mt-2">
                      <button
                        onClick={() => handleSelectTool("ai")}
                        className="w-full text-left px-4 py-3 rounded-[32px] hover:bg-[#FFFFFF]/10  text-gray-600 hover:text-gray-900  text-sm font-medium transition-colors flex items-center justify-between group bg-slate-50 "
                      >
            ...
            {!query && (
              <div className="border-t border-slate-100 p-4 bg-slate-50  flex items-center justify-between text-xs font-medium text-gray-600 ">
